defmodule NavatrackWeb.Aims.ShowTest do
  ...
  test "show", %{conn: conn} do
    ...
    assert response =~ "Mean Lead Time For Changes"
    assert response =~ "Actual Value: #{x.mean_lead_time_for_changes_actual}"
    assert response =~ "Target Value: #{x.mean_lead_time_for_changes_expect}"
    assert response =~ "Ratio: #{x.mean_lead_time_for_changes_ratio}"
    assert response =~ "Unit: #{x.mean_lead_time_for_changes_unit}"
    assert response =~ "Description: #{x.mean_lead_time_for_changes_description}"
    ...
    assert response =~ "Failed Deployment Recovery Time"
    assert response =~ "Actual Value: #{x.failed_deployment_recovery_time_actual}"
    assert response =~ "Target Value: #{x.failed_deployment_recovery_time_expect}"
    assert response =~ "Ratio: #{x.failed_deployment_recovery_time_ratio}"
    assert response =~ "Unit: #{x.failed_deployment_recovery_time_unit}"
    assert response =~ "Description: #{x.failed_deployment_recovery_time_description}"
