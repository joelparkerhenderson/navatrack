defmodule NavatrackWeb.PlaceReviews.FormLive do
  ...
  @doc """
  Load the place options suitable for the select input.
  """

  def place_options() do
    places = Navatrack.Works.Place |> Ash.read!()
    Enum.map(places, fn x ->
      {x.name, x.id}
    end)
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app {assigns}>
      ...
      >
        ...
        <.input
          field={form[:strength]}
          label="What is the place's key strength?"
        />
        <.input
          field={form[:start]}
          label="What should the place start doing in order to be effective?"
        />
        <.input
          field={form[:stop]}
          label="What should the place stop doing in order to be effective?"
        />
        <.input
          field={form[:continue]}
          label="What should the place continue doing in order to be effective?"
        />
        <.input
          field={form[:change]}
          label="What should the place change doing in order to be effective?"
        />
        <.input
          field={form[:advice]}
