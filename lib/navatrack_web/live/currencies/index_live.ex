defmodule NavatrackWeb.Currencies.IndexLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app {assigns}>
      ...
      <Cinder.Table.table theme={Cinder.Themes.Smart} resource={X}>
        ...
        <:col :let={x} field="number" label="Number" sort search>{x.number}</:col>
        <:col :let={x} field="symbol" label="Symbol" sort search>{x.symbol}</:col>
        <:col :let={x} field="position" label="Position" sort search>{x.position}</:col>
      </Cinder.Table.table>
    </Layouts.app>
    """
  end
