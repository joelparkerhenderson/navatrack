...
```sh
mix ash.gen.domain Navatrack.Works
mix ash.gen.resource Navatrack.Works.Task \
  --conflicts replace \
...
Create a tasks directory for views:

```sh
mkdir -p ~/git/sixarm/navatrack/lib/navatrack_web/live/tasks
```
...
```sh
--relationship "belongs_to:parent:Navatrack.Works.Task"
```
