defmodule Navatrack.Repo.Migrations.CreateTableAccessAttributes do
  ...
  def up do
    ...
    CREATE TABLE access_attributes (
      ...
      locale_code text,
      parent_id uuid CONSTRAINT access_attributes_parent_id_fk REFERENCES access_attributes (id),
      parent_order int CONSTRAINT access_attributes_parent_order_check CHECK (parent_order >= 0),
      --- card
      name text,
      sign text CONSTRAINT access_attributes_sign_check CHECK (LENGTH(sign) = 1),
      status text,
      tagging text,
      note text
    );
    ...
    execute "CREATE INDEX access_attributes_retired_at_index ON access_attributes (retired_at);"
  ...
  def down do
    execute "DROP CONSTRAINT IF EXISTS access_attributes_parent_id_fk;"
    execute "DROP CONSTRAINT IF EXISTS access_attributes_parent_order_check;"
    execute "DROP CONSTRAINT IF EXISTS access_attributes_sign_check;"
    ...
    execute "DROP INDEX IF EXISTS access_attributes_retired_at_index;"
